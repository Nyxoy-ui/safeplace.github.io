...
function normalizeDb() {
  ...
  db.books.forEach(book => {
    if (book.published === undefined) { book.published = true; changed = true; }
    if (!book.createdAt) { book.createdAt = book.updatedAt || new Date().toISOString(); changed = true; }
  });
...
function canWrite() { return !!session; }
function isMain() { return session?.role === 'main'; }
function canEditBook(book) { return canWrite() && (isMain() || book.authorId === session.id || (book.authorId === 'system' && session.role !== 'user')); }
function canReadBook(book) { return !!book.published || canEditBook(book); }
...
function renderDashboard() {
  ...
  app.innerHTML = `<section class="hero"><h1>Willkommen, ${escapeHtml(user.username)}.</h1><p>Dein Dashboard zeigt Bibliothek, Favoriten, zuletzt gelesene Bücher und ${canWrite() ? 'deine Schreibwerkzeuge.' : 'deine gespeicherten Antworten.'}</p><div class="grid three"><div class="stat-card"><strong>${db.books.filter(canReadBook).length}</strong><span>Bücher</span></div><div class="stat-card"><strong>${Object.keys(db.favorites[user.id] || {}).length}</strong><span>Favoriten</span></div><div class="stat-card"><strong>${Object.keys(db.answers[user.id] || {}).length}</strong><span>Antworten</span></div></div></section><section class="panel"><div class="toolbar"><h2>Zuletzt gelesen</h2><button class="primary-button" data-nav="library" type="button">Zur Bibliothek</button></div>${last ? bookGrid([last]) : '<div class="empty-state">Du hast noch kein Buch begonnen.</div>'}</section>`;
}
...
function renderLibrary(favoritesOnly = false) {
  ...
  let books = db.books.filter(book => canReadBook(book) && `${book.title} ${book.description}`.toLowerCase().includes(query));
...
function lastReadBooks() {
  ...
  return Object.entries(progress).sort((a,b) => (b[1].updatedAt || '').localeCompare(a[1].updatedAt || '')).map(([id]) => db.books.find(book => book.id === id)).filter(book => book && canReadBook(book));
}

function renderReader() {
  const book = db.books.find(item => item.id === view.bookId && canReadBook(item)) || db.books.find(canReadBook) || db.books[0];
...
function renderEditor() {
  if (!draftBook) {
    const requestedBook = db.books.find(book => book.id === view.editingBookId);
    const editableBook = requestedBook && canEditBook(requestedBook) ? requestedBook : db.books.find(book => canEditBook(book));
    draftBook = structuredClone(editableBook || createEmptyBook());
    view.editingBookId = draftBook.id;
  }
  const chapter = draftBook.chapters[view.chapterIndex] || draftBook.chapters[0];
  const page = chapter.pages[view.pageIndex] || chapter.pages[0];
  app.innerHTML = `<section class="panel"><div class="toolbar"><div><h2>Bücher schreiben</h2><p class="muted">Schreibe Bücher aus Kapiteln und Seiten. Fragen werden als Blöcke direkt zwischen Absätzen eingefügt. Entwürfe bleiben privat, bis du sie veröffentlichst.</p></div><div class="editor-actions"><span class="pill">${draftBook.published ? 'Veröffentlicht' : 'Entwurf'}</span><button class="secondary-button" data-new-book type="button">Neues Buch</button></div></div><div class="editor-layout"><aside class="editor-sidebar"><label>Buch<select id="bookSelect">${editorBookOptions()}</select></label><label>Titel<input id="bookTitle" value="${escapeHtml(draftBook.title)}"></label><label>Beschreibung<textarea id="bookDescription">${escapeHtml(draftBook.description)}</textarea></label><label>Cover URL oder Data-URL<textarea id="bookCover">${escapeHtml(draftBook.cover || '')}</textarea></label><div class="grid two"><label>Schriftart<select id="bookFont"><option ${draftBook.fontFamily.includes('Georgia') ? 'selected' : ''} value="Georgia, serif">Georgia</option><option ${draftBook.fontFamily.includes('Arial') ? 'selected' : ''} value="Arial, sans-serif">Arial</option><option ${draftBook.fontFamily.includes('Verdana') ? 'selected' : ''} value="Verdana, sans-serif">Verdana</option></select></label><label>Größe<input id="bookSize" type="number" min="14" max="32" value="${draftBook.fontSize}"></label></div><button class="primary-button" data-save-book type="button">Buch zwischenspeichern</button><button class="secondary-button" data-publish-book type="button">${draftBook.published ? 'Veröffentlichung aktualisieren' : 'Buch veröffentlichen'}</button><button class="secondary-button" data-add-chapter type="button">Kapitel hinzufügen</button><div class="chapter-list">${draftBook.chapters.map((ch, i) => `<button class="ghost-button" data-chapter="${i}" type="button">${i + 1}. ${escapeHtml(ch.title)}</button>`).join('')}</div></aside><div><label>Kapiteltitel<input id="chapterTitle" value="${escapeHtml(chapter.title)}"></label><div class="toolbar"><div class="page-list">${chapter.pages.map((pg, i) => `<button class="ghost-button" data-page="${i}" type="button">${escapeHtml(pg.title)}</button>`).join('')}</div><button class="secondary-button" data-add-page type="button">Seite hinzufügen</button></div><label>Seitentitel<input id="pageTitle" value="${escapeHtml(page.title)}"></label><div class="editor-actions"><button class="secondary-button" data-add-paragraph type="button">Absatz einfügen</button><button class="secondary-button" data-add-question type="button">Frage im Text einfügen</button></div><div class="block-list">${page.blocks.map((block, i) => renderEditBlock(block, i)).join('')}</div></div></div></section>`;
}
function editorBookOptions() {
  const editableBooks = db.books.filter(book => canEditBook(book));
  const options = editableBooks.map(book => `<option value="${book.id}" ${book.id === draftBook.id ? 'selected' : ''}>${escapeHtml(book.title)}</option>`);
  if (!editableBooks.some(book => book.id === draftBook.id)) options.unshift(`<option value="${draftBook.id}" selected>${escapeHtml(draftBook.title)} (neu)</option>`);
  return options.join('');
}
...
function persistDraft(message = 'Buch zwischengespeichert') { syncDraftFromForm(); draftBook.updatedAt = new Date().toISOString(); const i = db.books.findIndex(book => book.id === draftBook.id); if (i >= 0) db.books[i] = structuredClone(draftBook); else db.books.push(structuredClone(draftBook)); view.editingBookId = draftBook.id; saveDb(message); render(); }
function publishDraft() {
  syncDraftFromForm();
  if (!draftBook.title.trim()) return showToast('Bitte gib deinem Buch zuerst einen Titel.');
  if (!draftBook.chapters.some(chapter => chapter.pages.some(page => page.blocks.length))) return showToast('Bitte füge vor dem Veröffentlichen Inhalt hinzu.');
  draftBook.published = true;
  draftBook.publishedAt = draftBook.publishedAt || new Date().toISOString();
  persistDraft('Buch veröffentlicht');
}
...
app.addEventListener('click', event => {
  ...
  if (target.dataset.edit) { const book = db.books.find(item => item.id === target.dataset.edit); if (!book || !canEditBook(book)) return showToast('Du kannst nur eigene Bücher bearbeiten.'); view.name = 'editor'; view.editingBookId = target.dataset.edit; view.chapterIndex = 0; view.pageIndex = 0; draftBook = null; render(); }
  ...
  if (target.dataset.publishBook !== undefined) publishDraft();
